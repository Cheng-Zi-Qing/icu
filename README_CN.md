...
- 进入 `working` 会启动护眼提醒和喝水提醒。
- 进入 `focus` 会暂停提醒。
- 从 `focus` 或 `break` 回到工作态后会重新启动提醒。
- 当前 Swift 迁移版已经落地护眼提醒、喝水提醒，以及日报/周报健康报告；拉伸仍然属于后续路线。
...
下面这些研究链接描述的是更完整的产品方向。当前 Swift shell 里已经真实启用护眼提醒、喝水提醒，以及健康报告；拉伸和更丰富的教练能力仍然属于后续路线。
